...
URA0401 = ClassUnit(CAirUnit) {
    ...
    CreateEnhancement = function(self, enh)
        ...
        if enh == "AdvancedProtonAccelerator" then
            local wepr = self:GetWeaponByLabel('HeavyBolter')
            local wepl = self:GetWeaponByLabel('HeavyBolterBack')
            local mult = bp.DamageMult
            wepr:AddDamageMod((mult - 1) * wepr.Blueprint.Damage)
            wepl:AddDamageMod((mult - 1) * wepl.Blueprint.Damage)
