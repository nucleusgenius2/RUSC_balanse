...
URL0001 = ClassUnit(ACUUnit, CCommandUnit) {
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh == 'CoolingUpgrade' then
            ...
            oc:ChangeRateOfFire(bp.NewRateOfFire1 or 0.1)
            local aoc = self:GetWeaponByLabel('AutoOverCharge')
            aoc:ChangeMaxRadius(bp.NewMaxRadius or 30)
            aoc:ChangeRateOfFire(bp.NewRateOfFire1 or 0.1)
        ...
        elseif enh == 'CoolingUpgradeRemove' then
            local wep = self:GetWeaponByLabel('RightRipper')
            local wepBp = self.Blueprint.Weapon
            local bpDisrupt1 = self.Blueprint.Weapon[2].RateOfFire
            local oc = self:GetWeaponByLabel('OverCharge')
            oc:ChangeRateOfFire(bpDisrupt1 or 0.033333)
            local aoc = self:GetWeaponByLabel('AutoOverCharge')
            aoc:ChangeRateOfFire(bpDisrupt1 or 0.033333)
            for _, v in wepBp do
                if v.Label == 'RightRipper' then
        ...
        elseif enh == 'Overcharge1' then
            ...
            wepOC:ChangeRateOfFire(bp.NewRateOfFire1 or 0.3)
            wepAutoOC:ChangeRateOfFire(bp.NewRateOfFire1 or 0.3)
        elseif enh == 'Overcharge1Remove' then
            local wepOC = self:GetWeaponByLabel('OverCharge')
            local wepAutoOC = self:GetWeaponByLabel('AutoOverCharge')
            wepOC:AddDamageMod(bp.OverchargeDamageMod)
            wepAutoOC:AddDamageMod(bp.OverchargeDamageMod)
            wep:ChangeRateOfFire(bp.NewRateOfFire1 or 2)
            wepAutoOC:ChangeRateOfFire(bp.NewRateOfFire1 or 2)
        elseif enh == 'Overcharge2' then
            local wepOC = self:GetWeaponByLabel('OverCharge')
            local wepAutoOC = self:GetWeaponByLabel('AutoOverCharge')
            wepOC:AddDamageMod(bp.OverchargeDamageMod)
            wepAutoOC:AddDamageMod(bp.OverchargeDamageMod)
        elseif enh == 'Overcharge2Remove' then
            local wepOC = self:GetWeaponByLabel('OverCharge')
            local wepAutoOC = self:GetWeaponByLabel('AutoOverCharge')
            wepOC:AddDamageMod(bp.OverchargeDamageMod)
            wepAutoOC:AddDamageMod(bp.OverchargeDamageMod)
    ...
    OnIntelEnabled = function(self, intel)
        ...
        elseif self.StealthEnh and self:IsIntelEnabled('RadarStealth') then
    ...
    OnIntelDisabled = function(self, intel)
        ...
        if self.CloakEnh and not self:IsIntelEnabled('Cloak') then
            self:SetMaintenanceConsumptionInactive()
        elseif self.StealthEnh and not self:IsIntelEnabled('RadarStealth') then
            self:SetMaintenanceConsumptionInactive()
        end
    end,
    ...
    OnLayerChange = function(self, new, old)
        ...
        -- if new == "Seabed" then
        --     if self.StealthEnh or self.CloakEnh then
        --         self:RemoveToggleCap('RULEUTC_CloakToggle')
        --         self:DisableUnitIntel('Enhancement', 'RadarStealth')
