...
function TeleportChargingProgress(unit, fraction)
    ...
    if bp.Display.TeleportEffects.PlayChargeFxAtDestination ~= false then
        ...
        local faction = bp.General.FactionName
    --    local costs = bp.Economy.BuildCostMass
		...
		elseif EntityCategoryContains(categories.BIGTELEPORT, unit) then
			local scale = (2 * fraction - MathPow(fraction, 2)) * 4
            for _, fx in unit.TeleportDestChargeBag do
                IEffectScaleEmitter(fx, scale)
            end
	--	elseif costs == 65000 then
	--		local scale = (2 * fraction - MathPow(fraction, 2)) * 4
    --        for _, fx in unit.TeleportDestChargeBag do
    --            IEffectScaleEmitter(fx, scale)
    --        end
	--	elseif costs == 48000 then
	--		local scale = (2 * fraction - MathPow(fraction, 2)) * 4
    --        for _, fx in unit.TeleportDestChargeBag do
    --            IEffectScaleEmitter(fx, scale)
    --        end
