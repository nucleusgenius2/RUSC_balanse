...
Unit = ClassUnit(moho.unit_methods, IntelComponent, VeterancyComponent) {
    ...
    DoTakeDamage = function(self, instigator, amount, vector, damageType)
		...
		elseif instigatorHasLOWDAMAGE and bpID == 'ual0202' then
			self:AdjustHealth(instigator, -amount*0.7)
		elseif instigatorHasLOWDAMAGE and bpID == 'url0202' then
			self:AdjustHealth(instigator, -amount*0.7)
		elseif instigatorHasLOWDAMAGE and bpID == 'xsl0202' then
			self:AdjustHealth(instigator, -amount*0.7)
		elseif instigatorHasLOWDAMAGE and bpID == 'xel0305' then
			self:AdjustHealth(instigator, -amount*0.5)
		elseif instigatorHasLOWDAMAGE and bpID == 'ual0303' then
			self:AdjustHealth(instigator, -amount*0.5)
		elseif instigatorHasLOWDAMAGE and bpID == 'xrl0305' then
			self:AdjustHealth(instigator, -amount*0.5)
		elseif instigatorHasLOWDAMAGE and bpID == 'xsl0303' then
			self:AdjustHealth(instigator, -amount*0.5)
    ...
    OnHealthChanged = function(self, new, old)
        ...
        local aiPlatoon = self.AIPlatoonReference
        if aiPlatoon then
            aiPlatoon:OnHealthChanged(self, new, old)
        end
    end,
    ...
    CreateWreckageProp = function(self, overkillRatio)
        ...
        if layer == 'Water' or layer == 'Sub' then
			if tech_category ~= 'EXPERIMENTAL' then
				mass = mass * 0.6
				energy = energy * 0.6
			end	
        end
    ...
    OnStopBeingBuilt = function(self, builder, layer)
        ...
        if self.Blueprint.Intel.DisabledIntel then
            for intelType in self.Blueprint.Intel.DisabledIntel do
    ...
    OnStartSacrifice = function(self, targetUnit)
        local id = targetUnit:GetEntityId()
		if targetUnit:GetBlueprint().BlueprintId ~= 'ssb5401' then
            ...
            FloatingEntityText(id,'Запрещенный юнит')
		end
    end,
    ...
    OnTeleportUnit = function(self, teleporter, location, orientation)
        ...
        if maxRange and destRange > maxRange then
            FloatingEntityText(id,'Пункт назначения вне зоны досягаемости')
            return
        end
		if not self:HasEnhancement('AdvancedTeleporter') then
			...
			for num, brain in ArmyBrains do
                if  IsAlly(brain:GetArmyIndex(), army) then continue end

				local unitList = brain:GetListOfUnits(categories.ANTITELEPORT, false)
				for i, unit in unitList do
                    if not unit.GetAntiTeleActive or not unit:GetAntiTeleActive() then
                        continue
                    end
                    
                    local blockerRange = unit:GetBlueprint().Defense.NoTeleDistance
                    if not blockerRange then
                        continue
                    end
                
                    local blockerPosition = unit:GetPosition()
                    local targetDist = VDist2(location[1], location[3], blockerPosition[1], blockerPosition[3])
                    -- ForkThread(function ()
                    --     for i = 1,10 do
                    --         DrawCircle(blockerPosition, blockerRange, 'ffffffff')
                    --         WaitTicks(1)
                    --     end
                    -- end)
                    local sourceCheck = VDist2(myposition[1], myposition[3], blockerPosition[1], blockerPosition[3])
                    if blockerRange >= targetDist then
                        FloatingEntityText(id, 'Пункт назначения телепорта заблокирован')
                        return
                    -- elseif blockerRange and blockerRange >= sourceCheck then
                    -- 	FloatingEntityText(id, 'Местоположение источника телепортации заблокировано')
                    -- 	return
                    end
        ...
        if telecost > 0 and not self.TeleportCostPaid then
            if storedenergy >= telecost then
                ...
                FloatingEntityText(id,'Недостаточно энергии для Телепортации')
                return
            end
        end
