...
BuffEffects = {
    ...
    MaxHealth = function(buffDefinition, buffValues, unit, buffName)
        ...
        if not buffValues.DoNotFill and not unit.IsBeingTransferred then
            unit:SetHealth(unit, unit:GetHealth() / oldmax * val)
        end
    end,
    ...
    DummyBuff = function(buffDef, buffValues, unit, buffName, instigator, afterRemove)
    end
}
    ...
    for k, v in unit.Buffs.Affects[affectType] do
        ...
        if v.Mult then
            if affectType == 'Regen' then
                ...
                if v.Mult > 1 then WARN('Regen mult too high, should be <1, for unit ' ..
                        unit.UnitId .. ' and buff ' .. buffName)
                    return
                end
