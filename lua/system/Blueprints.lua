...
local function FindCustomStrategicIcons(all_bps)
    ...
    if preGameData and preGameData.IconReplacements then
        for _, info in preGameData.IconReplacements do
            ...
            local ok, msg = pcall(
                function()
                    ...
                    if not (state.UnitIconAssignments or state.ScriptedIconAssignments) then
                        error("Blueprints.lua - can not import the icon configuration file at '" ..
                            info.Location ..
                            "'. This could be due to missing functionality functionality or a parsing error.")
                    end
                end
            )
            ...
            if not ok then
                WARN("Blueprints.lua - Unable to load icons from mod '" ..
                    info.Name .. "' with uuid '" .. info.UID .. "'. Please inform the author: " .. info.Author)
                WARN(msg)
            end
            ...
            ok, msg = pcall(
                function()
                    ...
                    if state.ScriptedIconAssignments then
                        ...
                        if n > 0 then
                            SPEW("Blueprints.lua - Found (" ..
                                n .. ") scripted icon assignments in " .. info.Name .. " by " .. info.Author .. ".")
                        end
                    end
                    ...
                    if state.UnitIconAssignments then
                        ...
                        if n > 0 then
                            SPEW("Blueprints.lua - Found (" ..
                                n .. ") manual icon assignments in " .. info.Name .. " by " .. info.Author .. ".")
                        end
                    end
                end
            )
...
function HandleUnitWithBuildPresets(bps, all_bps)
    ...
    local sortCategories = { ['SORTOTHER'] = true, ['SORTINTEL'] = true, ['SORTSTRATEGIC'] = true, ['SORTDEFENSE'] = true,
        ['SORTECONOMY'] = true, ['SORTCONSTRUCTION'] = true, }
            ...
            if not preset.BuildCostEnergyOverride or not preset.BuildCostMassOverride or not preset.BuildTimeOverride then
                for _, enh in preset.Enhancements do
                    ...
                    if tempBp.Enhancements[enh] then
                        ...
                        WARN('*DEBUG: Enhancement ' ..
                            repr(enh) ..
                            ' used in preset ' ..
                            repr(name) .. ' for unit ' .. repr(tempBp.BlueprintId) .. ' does not exist')
                    end
                end
            end
...
function PreModBlueprints(all_bps)
    for _, bp in pairs(all_bps.Unit) do
        ...
        --disable stealth for units that shouldn't have it by default
        --workaround for cloak transport
        if bp.Intel then
            local idsToInclude = {
                ["xrl03021"] = true,
            }
            local catHash = bp.CategoriesHash
            if catHash.MOBILE and (catHash.TECH3 or catHash.SUBCOMMANDER or catHash.COMMAND)
                or idsToInclude[bp.BlueprintId] then
                local intel = bp.Intel
                intel.DisabledIntel = {}
                for _, intelType in { "Cloak", "RadarStealth" } do
                    if not intel[intelType] then
                        intel.DisabledIntel[intelType] = true
                        intel[intelType] = true
                    end
                end
            end
        end
...
local function SpawnMenuDummyChanges(all_bps)
    for id, bp in pairs(all_bps) do
        ...
        if bp.Physics and bp.Physics.MotionType ~= 'RULEUMT_Air' then
            ...
            local SOffsetX, SOffsetZ = NewOffset(GetOffset(bp, 'X'), FootX, ReducedFootX),
                NewOffset(GetOffset(bp, 'Z'), FootZ, ReducedFootZ)

            local DummyID = 'spawn_dummy_' .. SkirtX .. SkirtZ ..
                '_' .. SOffsetX .. SOffsetZ .. '_' .. ReducedFootX .. ReducedFootZ
